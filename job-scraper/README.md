...
- **多來源查詢**：**104 + Yourator**（新創職缺），可自選來源，依 `job_url` 去重
...
| `scraper_yourator.py` | Yourator API 客戶端（新創職缺）|
| `scraper_1111.py` | 1111 API 客戶端（captcha 停用中）|
...
- **104**：使用正式 JSON API `https://www.104.com.tw/jobs/search/api/jobs`（已實測可用）。
- **1111（目前停用）**：1111 的搜尋頁前面有反爬蟲驗證碼（`/api/v1/captcha-gate`），
  用 `requests` 這類無頭請求會被 404／captcha 擋下，需改用瀏覽器自動化（Selenium/Playwright）
  並處理驗證碼。因此預設只用 104；`scraper_1111.search_1111` 會丟出說明錯誤，
  原本的 API 呼叫流程保留在 `_search_1111_raw` 供日後接上瀏覽器自動化時參考。
